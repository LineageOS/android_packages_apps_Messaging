...
public class BugleDatabaseOperations {
    ...
    public static void setStarred(Uri contactUri, boolean value) {
        ...
        final Cursor c = resolver.query(contactUri, new String[]{ContactsContract.Contacts._ID},
                null, null, null);
        if (c == null) {
            return;
        }
        try {
            if (c.moveToFirst()) {
                ...
                if (id < ContactsContract.Profile.MIN_ID) {
                    ContactsContract.PinnedPositions.undemote(resolver, id);
                }
            }
        } finally {
            c.close();
